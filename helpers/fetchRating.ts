...
export async function setRecipeRating(recipeId: string, uid: string, value: number) {
    if (!uid) throw new Error('Missing uid');
    if (!Number.isInteger(value) || value < 1 || value > 5) throw new Error('Invalid rating value');
    ...
    await runTransaction(firestore, async (tx) => {
        ...
        if (ratingSnap.exists()) {
            const oldValue = (ratingSnap.data() as { value?: number }).value ?? 0;
            const delta = value - oldValue;

            tx.update(ratingRef, { value, updatedAt: serverTimestamp() });
            tx.update(recipeRef, { ratingSum: increment(delta) });
        } else {
            tx.set(ratingRef, { value, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
            tx.update(recipeRef, {
