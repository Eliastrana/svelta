...
const OnboardingIntro: React.FC<OnboardingIntroProps> = ({
    ...
    const [backgroundPhotoPreview, setBackgroundPhotoPreview] = useState(initialBackgroundPhotoURL);
    ...
    const finishOnboarding = async (skipCustomizations = false) => {
        ...
        try {
            ...
            const payload = {
                ...
                favoriteFood: skipCustomizations ? initialFavoriteFood : favoriteFood.trim(),
                photoURL: skipCustomizations ? initialPhotoURL : nextPhotoURL,
                backgroundPhotoURL: skipCustomizations ? initialBackgroundPhotoURL : nextBackgroundPhotoURL,
                profileThemeId: skipCustomizations ? initialProfileThemeId : profileThemeId,
                profileFontId: skipCustomizations ? initialProfileFontId : profileFontId,
                isProfilePrivate: skipCustomizations ? initialIsProfilePrivate : isProfilePrivate,
    ...
    return (
        <div className="fixed inset-0 z-[140] overflow-y-auto bg-[#fbfaf4] px-4 py-4 sm:px-5 sm:py-6">
            <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-2xl items-center justify-center">
                        ...
                        {STEPS.map((_, index) => (
                            <span
                                ...
                                style={{
                                    ...
                                    backgroundColor: index <= step ? activeTheme.main : '#d8dfd0',
                                }}
                            />
                        ))}
                    </div>
                ...
                <div className="text-center" style={activeStep.id === 'formatting' ? { fontFamily: activeFont.family } : undefined}>
                    <p className="text-xs font-semibold uppercase" style={{ color: activeTheme.accent }}>
                        {activeStep.eyebrow}
                    </p>
                ...
                {activeStep.id === 'features' ? (
                    <div className="mt-8 space-y-3 text-center">
                        <div className="rounded-2xl border border-[#ece8da] px-4 py-4 text-slate-700">Følg kokker</div>
                        <div className="rounded-2xl border border-[#ece8da] px-4 py-4 text-slate-700">Lagre oppskrifter</div>
                        <div className="rounded-2xl border border-[#ece8da] px-4 py-4 text-slate-700">Se populære retter</div>
                    </div>
                ) : null}
                    ...
                    <div className="mt-8 space-y-4">
                        <div className="grid gap-3 sm:grid-cols-2">
                            <label className="cursor-pointer overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 text-left transition hover:border-slate-300">
                                <div className="relative aspect-[4/3] w-full bg-[#f1eee4]">
                                    {photoPreview ? (
                                        <img src={photoPreview} alt="Valgt profilbilde" className="h-full w-full object-cover" />
                                    ) : (
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 bg-white/90 px-4 py-3">
                                        <span className="block text-sm font-semibold text-slate-900">Profilbilde</span>
                                        <span className="mt-1 block text-sm text-slate-500">
                                            {photoPreview ? 'Trykk for å bytte' : 'Trykk for å legge til'}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <label className="cursor-pointer overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 text-left transition hover:border-slate-300">
                                <div className="relative aspect-[4/3] w-full bg-[#f1eee4]">
                                    {backgroundPhotoPreview ? (
                                        <img src={backgroundPhotoPreview} alt="Valgt bakgrunnsbilde" className="h-full w-full object-cover" />
                                    ) : (
                                        <div className="grid h-full w-full place-items-center text-slate-400">
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 bg-white/90 px-4 py-3">
                                        <span className="block text-sm font-semibold text-slate-900">Bakgrunnsbilde</span>
                                        <span className="mt-1 block text-sm text-slate-500">
                                            {backgroundPhotoPreview ? 'Trykk for å bytte' : 'Trykk for å legge til'}
                                        </span>
                                    </div>
                                </div>
                                <input type="file" accept="image/*" className="hidden" onChange={handlePickBackground} />
                            </label>
                        </div>
                        ...
                        <label className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-left">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-slate-500">Godkjenn nye følgere</p>
                            </div>
                ...
                {activeStep.id === 'formatting' ? (
                    <div className="mt-8 space-y-5">
                            ...
                            {PROFILE_THEMES.map((theme) => (
                                <button
                                    ...
                                    className={[
                                        ...
                                        profileThemeId === theme.id ? 'border-slate-900 ring-2 ring-slate-900/10' : 'border-slate-200 hover:border-slate-300',
                                    ].join(' ')}
                                >
                                    <div className="h-10" style={{ backgroundColor: theme.main }} />
                                    <div className="px-3 py-2 text-sm font-semibold" style={{ backgroundColor: theme.soft, color: theme.text }}>
                                        {theme.label}
                                    </div>
                                </button>
                            ))}
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                            {PROFILE_FONTS.map((font) => (
                                <button
                                    key={font.id}
                                    type="button"
                                    onClick={() => setProfileFontId(font.id)}
                                    className={[
                                        'rounded-2xl border px-4 py-3 text-center transition',
                                        profileFontId === font.id ? 'border-slate-900 bg-slate-50 ring-2 ring-slate-900/10' : 'border-slate-200 hover:border-slate-300',
                                    ].join(' ')}
                        ...
                        <div
                            ...
                            style={{ backgroundColor: activeTheme.soft, color: activeTheme.text, fontFamily: activeFont.family }}
                        >
                            <div className="h-28" style={{ backgroundColor: activeTheme.soft }}>
                                {backgroundPhotoPreview ? (
                                    <img src={backgroundPhotoPreview} alt="Bakgrunn" className="h-full w-full object-cover" />
                                ) : null}
                            </div>
                            <div className="px-4 py-4" style={{ backgroundColor: activeTheme.main, color: activeTheme.text }}>
                                <div className="flex items-center gap-3">
                                    <div className="h-14 w-14 overflow-hidden rounded-2xl" style={{ backgroundColor: `${activeTheme.text}14` }}>
                                        {photoPreview ? (
                                            <img src={photoPreview} alt="Profil" className="h-full w-full object-cover" />
                                        ) : (
                                            <div className="grid h-full w-full place-items-center text-2xl">🧑‍🍳</div>
                                        )}
                                    </div>
                                    <div className="min-w-0 text-left">
                                        <p className="truncate text-lg font-semibold">{initialName}</p>
                                        <p className="truncate text-sm" style={{ color: `${activeTheme.text}d9` }}>{favoriteFood.trim() || 'Favorittmat'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="mt-8 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button
                        ...
                        disabled={saving}
                    >
                        {isLastStep ? (saving ? 'Gjør klart…' : 'Start') : 'Fortsett'}
                    </button>
                </div>
            </div>
            </div>
